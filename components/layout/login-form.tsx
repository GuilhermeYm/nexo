...
interface LoginFormProps {
  /** Vem de "Trocar de conta" nas Configurações: poupa redigitar o e-mail da
   *  conta lembrada, nunca a senha. Lido no servidor (`?email=`) para não
   *  abrir uma fronteira de Suspense por um campo pré-preenchido. */
  prefillEmail: string;
}
...
export function LoginForm({ prefillEmail }: LoginFormProps) {
  ...
  async function onSubmit(values: LoginFormValues) {
    ...
    try {
      ...
      // O nome de exibição chega depois, na primeira visita às Configurações
      // — aqui só id e e-mail já bastam para a conta aparecer no seletor.
      rememberAccount({ id: user.id, email: user.email });
